...
export default function AddSize() {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post(API_BASE_URL + SIZE_URL + "/create", sizeData).then(
      (success) => {
  ...
  return (
    <div className="container mx-auto p-8">
      ...
      <form onSubmit={handleSubmit} className="p-8  rounded-lg space-y-8 text-white"
      style={{
        background:
        ...
        <div className='col-span-1'>
          ...
          onChange={createSlug}
            ...
            ref={sizeLabel}
            required
            options={Array.isArray(sizeOptions)
               &&
              sizeOptions?.map(
                (size, index) => {
                  return(
        ...
        {/* Hip */}
        <div className='col-span-1'>
          <label className="block text-sm font-medium mb-1">Hip (inches)</label>
          <input
            type="number"
            name="hip"
            ref={hip}
        ...
        <div className='col-span-1'>
          ...
          <Select styles={customStyles}
            ...
            required
            ref={indiaSize}
            options={Array.isArray(indiaSizeOptions)
               &&
              indiaSizeOptions?.map(
                (size, index) => {
                  return(
                    { value: size, label: size }
                  )
        ...
        {/* Intl Size */}
        <div className='col-span-1'>
          <label className="block text-sm font-medium mb-1">International Size</label>
          <Select styles={customStyles}
            name="intlSize"
            required
            ref={intlSize}
            options={Array.isArray(intlSizeOptions)
               &&
              intlSizeOptions?.map(
                (size, index) => {
                  return(
