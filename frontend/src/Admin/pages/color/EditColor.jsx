...
export default function EditColor() {
  const { toastNotify, API_BASE_URL, COLOR_URL, allColor, fetchAllColor } =
  ...
  const editColor = (event) => {
    ...
    axios
      .put(API_BASE_URL + COLOR_URL + "/edit/" + color_id, data)
      .then((success) => {
  ...
  return (
    <div
      className="max-w-xl mx-auto p-6 shadow rounded-2xl text-white mt-10"
      style={{
        background:
          "linear-gradient(145deg, #1a1a1a 0%, #000000 50%, #1a1a1a 75%, #2e2e2e 100%)",
      }}
    >
      <h2 className="text-2xl font-semibold mb-4 text-white">Edit Color</h2>
      <form onSubmit={editColor}>
        ...
        <div className="mb-4">
          <label className="block font-medium mb-2">Pick a Color</label>
          <input
        ...
        <button
          type="submit"
          className="w-full font-medium py-2 rounded-lg bg-neutral-600 addButton"
        >
          Update Color
        </button>
      </form>
